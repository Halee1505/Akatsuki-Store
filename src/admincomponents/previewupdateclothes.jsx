    ...
    return (
        <div className="card d-flex flex-column align-items-center" style={{ width: "18vw" }}>
            ...
            <div className="card-body" style={{ width: "17vw" }}>
                ...
                <p className="card-text">Size:</p>
                {
                    AddClothes.updateSize.length !== 0 ?
                        <div className="row d-flex justify-content-between">
                            {
                                (Object.keys(AddClothes.updateSize[countcolor]).length-2)>=2 && (Object.keys(AddClothes.updateSize[countcolor]).length-2)%2===0?
                                new Array((Object.keys(AddClothes.updateSize[countcolor]).length-2)/2).fill(0).map((item,index)=>{
                                    return <button key={index} className="btn" onClick={()=>{setCountSize(index)}}>{AddClothes.updateSize[countcolor]["classifycolor"+countcolor+"-"+index]}</button>
                                }):""
                            }
                        </div> :
                        <div className="row d-flex justify-content-between">
                ...
                <p className="card-text mt-3">Count: {AddClothes.updateSize.length !== 0?AddClothes.updateSize[countcolor]["countcolor"+countcolor+"-"+countsize]:0}</p>
            </div>
        </div>
    )
}